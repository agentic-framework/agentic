...
def main():
    ...
    update_packages_parser = subparsers.add_parser("update-packages", help="Update the package list for a virtual environment")
    update_packages_parser.add_argument("--venv-path", help="Path to the virtual environment")
    update_packages_parser.add_argument("--project-name", help="Name of the project")
    
    # Cleanup command
    subparsers.add_parser("cleanup", help="Remove entries for virtual environments that no longer exist")
    
    # Repair command
    subparsers.add_parser("repair", help="Attempt to repair the registry by scanning for virtual environments")
    
    # Backup command
    subparsers.add_parser("backup", help="Create a backup of the registry file")
    
    args = parser.parse_args()
    
    if args.command == "add":
        add_venv(args.venv_path, args.project_name, args.python_version, args.description, args.verify)
    elif args.command == "remove":
        remove_venv(args.venv_path, args.project_name)
    elif args.command == "list":
        list_venvs(args.verbose, args.packages)
        ...
        update_packages(args.venv_path, args.project_name)
        ...
        if backup_registry():
            ...
            print("Failed to create registry backup")
    else:
        parser.print_help()
