...
def check_utility_scripts():
    ...
    for script in UTILITY_SCRIPTS:
        ...
        if exists and executable:
            ...
            logger.info(f"Utility script exists and is executable: {script}")
        elif exists:
            print_status(f"Utility script: {script}", False, f"Script exists but is not executable. Run 'chmod +x {script_path}'")
            logger.warning(f"Script exists but is not executable: {script_path}")
            
            # Try to make it executable
            try:
                os.chmod(script_path, 0o755)
                print_status(f"Made script executable: {script}", True)
                logger.info(f"Made script executable: {script_path}")
                all_exist = True
            except Exception as e:
                print_status(f"Failed to make script executable: {script}", False, str(e))
                logger.error(f"Failed to make script executable {script_path}: {e}")
                all_exist = False
        else:
        ...
        if result.returncode == 0:
            ...
            if python_versions:
                ...
                if response == 'y':
                    ...
                    result = run_command("./uv_manager.py install-python", capture_output=False)
    ...
    try:
        ...
        if invalid_count > 0:
            ...
            if response == 'y':
                ...
                result = run_command("./venv_manager.py cleanup", capture_output=False)
                if result is not None:
...
def fix_environment():
    ...
    for script in UTILITY_SCRIPTS:
        ...
        if os.path.exists(script_path) and not os.access(script_path, os.X_OK):
            try:
                os.chmod(script_path, 0o755)
                fixes_applied.append(f"Made script executable: {script}")
                logger.info(f"Made script executable: {script_path}")
            except Exception as e:
                logger.error(f"Failed to make script executable {script_path}: {e}")
...
def main():
    ...
    if all_ok:
        ...
        if not venvs_ok:
            print("- Clean up invalid virtual environments using './venv_manager.py cleanup'")
