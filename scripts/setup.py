...
This script provides utilities for setting up the Agentic framework.
...
def make_scripts_executable(args):
    """Make utility scripts executable."""
    print("\n\033[1mMaking Utility Scripts Executable\033[0m\n")
    logger.info("Making utility scripts executable")
    
    # Utility scripts to make executable
    utility_scripts = [
        "ag",
        "scripts/venv_manager.py",
        "scripts/create_project.py",
        "scripts/uv_manager.py",
    ...
    for script in utility_scripts:
        ...
        if os.path.exists(script_path):
            try:
                ...
                print_status(f"Made executable: {script}", True)
                logger.info(f"Made executable: {script_path}")
            except Exception as e:
                print_status(f"Failed to make executable: {script}", False, str(e))
                logger.error(f"Failed to make executable {script_path}: {e}")
                all_executable = False
        else:
            print_status(f"Script not found: {script}", False)
            logger.warning(f"Script not found: {script_path}")
            all_executable = False
    
    return 0 if all_executable else 1
...
def setup_all(args):
    ...
    executable_result = make_scripts_executable(args)
    ...
    print_status("Make scripts executable", executable_result == 0)
    
    all_success = (install_result == 0 and create_result == 0 and 
                  registry_result == 0 and executable_result == 0)
    ...
    if all_success:
        ...
        print("  ./ag setup make-scripts-executable")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Usage: ./setup.py <command> [options]")
        print("\nAvailable commands:")
        ...
        print("  make-scripts-executable  Make utility scripts executable")
    ...
    if command == "install-dependencies":
        ...
        return create_directories(args)
    elif command == "initialize-registry":
        return initialize_registry(args)
    elif command == "make-scripts-executable":
        return make_scripts_executable(args)
    elif command == "all":
